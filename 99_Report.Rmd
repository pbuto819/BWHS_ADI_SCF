...
We examined the association of multidimensional ADI with Black women's self reported cognitive functioning. We hypothesized that Black women in more disadvantaged areas will have poorer subjective congitive functioning.  
...
We first present some descriptives about the cohort
...
# Modeling approaches {.tabset}

We modeled the outcome as a continuous variable, ordinal variable, and multinomial variable. Each of these approaches have advantages and disadvantages. The continuous may be the easiest/most intuitive to interpret, however, it may not capture the true form of the outcome. The Ordinal approach takes advantage of the natural ordering of the cognitive outcome levels. The multinomial approach was previuosly used in another study but may  not be sufficiently powered here. 

## Figure 1: Distribution of ADI
```{r, message=FALSE}
...
## Figure 2: Distribution of SCI 
```{r, message=FALSE}
data %>% ggplot(aes(cogsum)) + 
  theme_bw() + 
  geom_histogram() + 
  xlab("Total sum of subjective cognitive function")
```

## Figure 3: Unadjusted association
```{r, message=FALSE}
...
For each outcome model, we also controlled for three different sets of covariates. 

- The first set adjusted for age, highest parental education, and whether the participant received public assistance as a child. 
